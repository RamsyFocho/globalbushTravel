...
const FlightCategory = ({category,currentCategory,setCategory}: {currentCategory : string,category : string,setCategory : (value : string) => void}) => {
  return  <button onClick={() => setCategory(category)}>
            <span
              className={`text-[16px] pb-[7px] capitalize  ${category==currentCategory && "after:content-[''] after:-bottom-4  after:w-full after:h-1 after:block after:bg-red-500"} `}
            >
             {category}
            </span>
          </button>
    ...
    <div>
      <div>
       <div className="flex gap-6 text-white pb-5 pl-[27px]">
        {flightCategories.map((e) => <FlightCategory key={e} category={e} setCategory={setCategory} currentCategory={category}/>)}
          </div>
        <div className="bg-white lg:rounded-full max-lg:gap-2 lg:flex-row  rounded-md  shadow-lg flex flex-col lg:items-center justify-between p-2 space-y-2 md:space-y-0 lg:space-x-2 w-full max-w-5xl">
          ...
          {/* Going To */}
          <div className="flex-1  min-w-0 px-4 py-2 max-lg:border max-lg:rounded-md flex flex-col justify-center rounded-full md:border-r border-gray-200">
            <label
              htmlFor="goingTo"
              className="text-gray-600 text-sm font-semibold mb-1"
            >
            ...
            <input
              ...
              placeholder={goingTo}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                setGoingTo(e.target.placeholder)
              }
            />
          </div>
